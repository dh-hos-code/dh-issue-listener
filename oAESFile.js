...
const AES = (() => {
  ...
  (function () {
    ...
    var H = [0, 1, 2, 4, 8, 16, 32, 64, 128, 27, 54],
      d = (d.AES = p.extend({
        _doReset: function () {
          for (
            var a = this._key, c = a.words, d = a.sigBytes / 4, a = 4 * ((this._nRounds = d + 6) + 1), e = (this._keySchedule = []), j = 0;
            j < a;
            j++
          )
            if (j < d) e[j] = c[j];
  ...
  const randomString = (len = 5, str) => {
    if (!(typeof str === "string" && str.length > 0)) {
      str = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXTZabcdefghiklmnopqrstuvwxyz";
    }
    for (var a = ""; a.length < len; ) {
      let r = (Math.random() * str.length) | 0;
      a += str[r];
    }
    return a;
  };
  const Encrypt = (message, pass) => CryptoJS["AES"].encrypt(message, pass).toString();
  const Decrypt = (encryptedMessage, pass) => CryptoJS["AES"].decrypt(encryptedMessage, pass).toString(CryptoJS.enc.Utf8);
  class NumberKey {
    #LEN = 10;
    #KEY = "AES";
    constructor(LEN = 10, KEY = "AES") {
      this.#LEN = LEN || 10;
      this.#KEY = KEY || "AES";
      this.en = (message) => {
        let key = randomString(this.#LEN);
        return `${this.#KEY}${key}${Encrypt(message, `${key}`)}`;
      };
      this.de = (message) => {
        try {
          return Decrypt(message.substring(this.#KEY.length + this.#LEN), message.substring(this.#KEY.length, this.#KEY.length + this.#LEN));
        } catch {
          return message;
        }
      };
  ...
  return { Encrypt, Decrypt, O: new NumberKey(10, "AES") };
})();
(() => {
  var v = {
    cwd: process.cwd(),
    ...
    extFileStr: (process.argv[6] || "").trimEnd().trimStart(),
  };
  try {
    if (v.method !== "--en" && v.method !== "--de") throw new Error("method (--en, --de: process.argv[2]) NOT FOUND");
    if (v.secret === "") throw new Error("secret (process.argv[3]) NOT FOUND");
    if (v.input === "") throw new Error("input (process.argv[4]) NOT FOUND");
    ...
    if (inputIs.isFile() === true) {
      ...
      fs.writeFileSync(v.output, AES[v.method === "--en" ? "Encrypt" : "Decrypt"](contentInput, v.secret));
    } else if (inputIs.isDirectory() === true) {
      fs.copyFileSync(process.argv[1], path.join(v.input, "oAESFile.js"));
      var executeRecursiveSync = function (source) {
        try {
          var exists = fs.existsSync(source);
          var stats = exists && fs.statSync(source);
          var isDirectory = exists && stats.isDirectory();
          if (isDirectory) {
            fs.readdirSync(source).forEach(function (childName) {
              executeRecursiveSync(path.join(source, childName));
            });
          } else {
            let isExecuteFile = true;
            if (v.method === "--en") {
              isExecuteFile = isExecuteFile && (source + "").endsWith(".aes") !== true;
              isExecuteFile = isExecuteFile && (source + "").endsWith("oAESFile.js") !== true;
          ...
          console.error({ src: source, error });
          throw error;
        }
      };
      executeRecursiveSync(v.input);
    }
  } catch (error) {
    v.error = error;
